...
{
	cpu->eax = OS_SWAP_INT32(cpu->eax);
	cpu->ebx = OS_SWAP_INT32(cpu->ebx);
	cpu->ecx = OS_SWAP_INT32(cpu->ecx);
	cpu->edx = OS_SWAP_INT32(cpu->edx);
	cpu->edi = OS_SWAP_INT32(cpu->edi);
	cpu->esi = OS_SWAP_INT32(cpu->esi);
	cpu->ebp = OS_SWAP_INT32(cpu->ebp);
	cpu->esp = OS_SWAP_INT32(cpu->esp);
	cpu->ss = OS_SWAP_INT32(cpu->ss);
	cpu->eflags = OS_SWAP_INT32(cpu->eflags);
	cpu->eip = OS_SWAP_INT32(cpu->eip);
	cpu->cs = OS_SWAP_INT32(cpu->cs);
	cpu->ds = OS_SWAP_INT32(cpu->ds);
	cpu->es = OS_SWAP_INT32(cpu->es);
	cpu->fs = OS_SWAP_INT32(cpu->fs);
	cpu->gs = OS_SWAP_INT32(cpu->gs);
}
...
enum NXByteOrder target_byte_sex)
{
	cpu->rax = OS_SWAP_INT64(cpu->rax);
	cpu->rbx = OS_SWAP_INT64(cpu->rbx);
	cpu->rcx = OS_SWAP_INT64(cpu->rcx);
	cpu->rdx = OS_SWAP_INT64(cpu->rdx);
	cpu->rdi = OS_SWAP_INT64(cpu->rdi);
	cpu->rsi = OS_SWAP_INT64(cpu->rsi);
	cpu->rbp = OS_SWAP_INT64(cpu->rbp);
	cpu->rsp = OS_SWAP_INT64(cpu->rsp);
	cpu->rflags = OS_SWAP_INT64(cpu->rflags);
	cpu->rip = OS_SWAP_INT64(cpu->rip);
	cpu->r8 = OS_SWAP_INT64(cpu->r8);
	cpu->r9 = OS_SWAP_INT64(cpu->r9);
	cpu->r10 = OS_SWAP_INT64(cpu->r10);
	cpu->r11 = OS_SWAP_INT64(cpu->r11);
	cpu->r12 = OS_SWAP_INT64(cpu->r12);
	cpu->r13 = OS_SWAP_INT64(cpu->r13);
	cpu->r14 = OS_SWAP_INT64(cpu->r14);
	cpu->r15 = OS_SWAP_INT64(cpu->r15);
	cpu->cs = OS_SWAP_INT64(cpu->cs);
	cpu->fs = OS_SWAP_INT64(cpu->fs);
	cpu->gs = OS_SWAP_INT64(cpu->gs);
}
...
enum NXByteOrder target_byte_sex)
{
	hdr->flavor = OS_SWAP_INT32(hdr->flavor);
	hdr->count = OS_SWAP_INT32(hdr->count);
}
...
enum NXByteOrder target_byte_sex)
{
	...
	fpu->fpu_reserved[0] = OS_SWAP_INT32(fpu->fpu_reserved[0]);
	fpu->fpu_reserved[1] = OS_SWAP_INT32(fpu->fpu_reserved[1]);

	if(target_byte_sex == host_byte_sex){
	    memcpy(&sfpc, &(fpu->fpu_fcw),
		   sizeof(struct swapped_fp_control));
	    sfpc.u.half = OS_SWAP_INT16(sfpc.u.half);
	    ...
	    sfps.u.half = OS_SWAP_INT16(sfps.u.half);
	...
	fpu->fpu_fop = OS_SWAP_INT16(fpu->fpu_fop);
	fpu->fpu_ip = OS_SWAP_INT32(fpu->fpu_ip);
	fpu->fpu_cs = OS_SWAP_INT16(fpu->fpu_cs);
	fpu->fpu_rsrv2 = OS_SWAP_INT16(fpu->fpu_rsrv2);
	fpu->fpu_dp = OS_SWAP_INT32(fpu->fpu_dp);
	fpu->fpu_ds = OS_SWAP_INT16(fpu->fpu_ds);
	fpu->fpu_rsrv3 = OS_SWAP_INT16(fpu->fpu_rsrv3);
	fpu->fpu_mxcsr = OS_SWAP_INT32(fpu->fpu_mxcsr);
	fpu->fpu_mxcsrmask = OS_SWAP_INT32(fpu->fpu_mxcsrmask);
	fpu->fpu_reserved1 = OS_SWAP_INT32(fpu->fpu_reserved1);
}
...
enum NXByteOrder target_byte_sex)
{
	exc->trapno = OS_SWAP_INT32(exc->trapno);
	exc->err = OS_SWAP_INT32(exc->err);
    	exc->faultvaddr = OS_SWAP_INT64(exc->faultvaddr);
}
...
enum NXByteOrder target_byte_sex)
{
	debug->dr0 = OS_SWAP_INT32(debug->dr0);
	debug->dr1 = OS_SWAP_INT32(debug->dr1);
	debug->dr2 = OS_SWAP_INT32(debug->dr2);
	debug->dr3 = OS_SWAP_INT32(debug->dr3);
	debug->dr4 = OS_SWAP_INT32(debug->dr4);
	debug->dr5 = OS_SWAP_INT32(debug->dr5);
	debug->dr6 = OS_SWAP_INT32(debug->dr6);
	debug->dr7 = OS_SWAP_INT32(debug->dr7);
}
...
enum NXByteOrder target_byte_sex)
{
	debug->dr0 = OS_SWAP_INT64(debug->dr0);
	debug->dr1 = OS_SWAP_INT64(debug->dr1);
	debug->dr2 = OS_SWAP_INT64(debug->dr2);
	debug->dr3 = OS_SWAP_INT64(debug->dr3);
	debug->dr4 = OS_SWAP_INT64(debug->dr4);
	debug->dr5 = OS_SWAP_INT64(debug->dr5);
	debug->dr6 = OS_SWAP_INT64(debug->dr6);
	debug->dr7 = OS_SWAP_INT64(debug->dr7);
}
...
enum NXByteOrder target_byte_sex)
{
	exc->trapno = OS_SWAP_INT32(exc->trapno);
	exc->err = OS_SWAP_INT32(exc->err);
    	exc->faultvaddr = OS_SWAP_INT32(exc->faultvaddr);
}
...
enum NXByteOrder target_byte_sex)
{
	...
	fpu->environ.ip = OS_SWAP_INT32(fpu->environ.ip);
	fpu->environ.opcode = OS_SWAP_INT16(fpu->environ.opcode);
	fpu->environ.dp = OS_SWAP_INT32(fpu->environ.dp);
	...
	if(target_byte_sex == host_byte_sex){
	    ...
	    sfpt.u.half = OS_SWAP_INT16(sfpt.u.half);
	    ...
	    ss.u.half = OS_SWAP_INT16(ss.u.half);
		...
		fpu->stack.ST[i].mant = OS_SWAP_INT16(sfpd.mant);
		fpu->stack.ST[i].mant1 = OS_SWAP_INT16(sfpd.mant1);
		fpu->stack.ST[i].mant2 = OS_SWAP_INT16(sfpd.mant2);
		fpu->stack.ST[i].mant3 = OS_SWAP_INT16(sfpd.mant3);
		sfpd.u.half = OS_SWAP_INT16(sfpd.u.half);
	    ...
	    for(i = 0; i < 8; i++){
		sfpd.mant = OS_SWAP_INT16(fpu->stack.ST[i].mant);
		sfpd.mant1 = OS_SWAP_INT16(fpu->stack.ST[i].mant1);
		sfpd.mant2 = OS_SWAP_INT16(fpu->stack.ST[i].mant2);
		sfpd.mant3 = OS_SWAP_INT16(fpu->stack.ST[i].mant3);
...
enum NXByteOrder target_byte_sex)
{
	...
	if(exc->trapno == 14){
		...
		sec.u.word = OS_SWAP_INT32(sec.u.word);
		...
		exc->err.normal.index = OS_SWAP_INT32(word);
		...
		sec.u.normal.index = OS_SWAP_INT32(word);
		sec.u.word = OS_SWAP_INT32(sec.u.word);
		memcpy(&(exc->err), &sec, sizeof(struct swapped_err_code));
	    }
	}
...
enum NXByteOrder target_byte_sex)
{
	user->self = OS_SWAP_INT32(user->self);
}
#endif /* i386_THREAD_STATE == -1 */
#endif /* !defined(RLD) */
